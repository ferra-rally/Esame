...
public class Calculator2 extends Application {
	...
	String[] buttonLabels = { "1", "2", "3", "+", "4", "5", "6", "-", "7", "8", "9", "X", "^1/2", "0", ".", "/", "=",
			"C" };	// button labels
	...
	private void setButton() {
		...
		for (i = 0; i < n; i++) {
			...
			} else if (buttonLabels[x].equals("^1/2")) {
				buttonArray[x].setOnAction(new EventHandler<ActionEvent>() {
					public void handle(ActionEvent event) {
						Double bfloat;
						...
						if (text != und) {
							if (text != "0") {
								b = calcLabel.getText();
								bfloat = Double.parseDouble(b);
								bfloat = Math.pow(bfloat, 0.5);
								String result = bfloat.toString();
								calcLabel.setText(result);
							}
						}
					}
